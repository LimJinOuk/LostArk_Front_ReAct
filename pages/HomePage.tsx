...
import {
    ...
    Clock,
    X,
    BarChart as BarChartIcon, // 이름을 바꿈
    RotateCcw,
    History,
    Book,
    Box
...
import {
    BarChart, // 이제 이 BarChart는 recharts의 컴포넌트로 인식됩니다.
    Bar,
    Cell,
    XAxis,
    YAxis,
...
const HomePage: React.FC = () => {
    ...
    const scrollRef = useRef<HTMLDivElement>(null); // 모바일 스크롤용
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center min-h-screen pt-10 mx-auto max-w-7xl px-4 lg:px-6 pb-20">
            ...
            <div className="text-center mb-8 lg:mb-12">
                <h1 className="text-[50px] lg:text-[80px] font-black tracking-tighter text-white mb-2 leading-none">LOAPANG</h1>
                <p className="text-zinc-400 text-sm lg:text-lg font-medium tracking-tight">로스트아크 실시간 시세 및 전적 검색</p>
            </div>

            {/* 메인 검색창 */}
            <div className="w-full max-w-2xl relative group mb-10 lg:mb-16" ref={historyRef}>
                <div className="absolute -inset-1 bg-gradient-to-r from-violet-600 to-indigo-900 rounded-[1.5rem] lg:rounded-[2.5rem] blur opacity-25 group-hover:opacity-50 transition duration-1000" />
                <form onSubmit={handleSearch} className="relative z-20">
                    <input
                        type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                        onFocus={() => setIsHistoryOpen(true)} placeholder="캐릭터명을 입력하세요"
                        className="w-full h-16 lg:h-20 px-6 lg:px-10 rounded-2xl lg:rounded-[1.5rem] bg-zinc-900/90 border-2 border-white/10 focus:border-indigo-500 outline-none text-xl lg:text-2xl font-bold text-white shadow-2xl transition-all"
                    />
                    <button type="submit" className="absolute right-2 top-1/2 -translate-y-1/2 w-12 h-12 lg:w-14 lg:h-14 bg-white text-black rounded-xl lg:rounded-[1.5rem] hover:scale-105 active:scale-95 transition-all flex items-center justify-center">
                        <Search
                            strokeWidth={3}
                            className="w-6 h-6 lg:w-7 lg:h-7" // 여기서 w-6(24px), lg:w-7(28px)로 조절합니다.
                        />
                    </button>
                </form>
                <AnimatePresence>
                    {isHistoryOpen && history.length > 0 && (
                        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="absolute top-[110%] left-0 right-0 z-50 bg-zinc-900 border border-white/10 rounded-2xl shadow-2xl overflow-hidden py-2">
                            {history.map((term, index) => (
                                <div key={index} onClick={() => handleSearch(term)} className="flex items-center justify-between px-6 py-3 hover:bg-white/5 cursor-pointer transition-colors">
            ...
            {/* 메인 콘텐츠 그리드 (모바일 세로 / PC 가로) */}
            <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 items-stretch min-h-0 lg:h-[500px] max-w-[1000px]">

                {/* 좌측: 아이템 목록 */}
                <div className="bg-zinc-900/50 border border-white/5 rounded-[2rem] lg:rounded-[2.5rem] flex flex-col overflow-hidden shadow-2xl backdrop-blur-sm h-[450px] lg:h-full">
                    <div className="flex border-b border-white/5 bg-white/5 p-2 gap-2 shrink-0">
                        <button onClick={() => { setActiveTab('material'); setListSearchTerm(""); }} className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl lg:rounded-2xl transition-all font-bold text-sm lg:text-base ${activeTab === 'material' ? 'bg-indigo-600 text-white shadow-lg' : 'text-zinc-500 hover:bg-white/5'}`}>
                            <Box size={16} /> 융화 재료
                        </button>
                        <button onClick={() => { setActiveTab('engraving'); setListSearchTerm(""); }} className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl lg:rounded-2xl transition-all font-bold text-sm lg:text-base ${activeTab === 'engraving' ? 'bg-indigo-600 text-white shadow-lg' : 'text-zinc-500 hover:bg-white/5'}`}>
                            <Book size={16} /> 각인서
                        </button>
                        <button onClick={fetchAndSaveData} className={`px-4 ${isRefreshing ? 'animate-spin' : ''} text-zinc-500`}><RotateCcw size={18} /></button>
                    </div>

                    <div className="px-4 py-3 bg-white/[0.02] border-b border-white/5 shrink-0">
                        <div className="relative">
                            <Search size={14} className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-500" />
                            <input type="text" value={listSearchTerm} onChange={(e) => setListSearchTerm(e.target.value)} placeholder={`${activeTab === 'material' ? '재료명' : '각인명'} 검색...`} className="w-full bg-zinc-800/50 border border-white/10 rounded-xl py-2 pl-10 pr-4 text-sm text-white focus:outline-none focus:border-indigo-500" />
                        </div>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto divide-y divide-white/5 custom-scrollbar min-h-0">
                        {filteredData.length > 0 ? (
                            filteredData.map((item) => (
                                <div key={item.Id} onClick={() => { setSelectedItem(item); if(window.innerWidth < 1024) scrollRef.current?.scrollIntoView({ behavior: 'smooth' }); }} className={`flex items-center justify-between px-6 py-4 cursor-pointer transition-all ${selectedItem?.Id === item.Id ? 'bg-indigo-500/10 border-l-4 border-indigo-500' : 'hover:bg-white/[0.02] border-l-4 border-transparent'}`}>
                                    <div className="flex items-center gap-4">
                                        <img src={item.Icon} className="w-10 h-10 lg:w-12 lg:h-12 bg-zinc-800 rounded-xl p-1" alt="" />
                                        <div>
                                            <p className={`font-bold text-sm ${item.Grade === '전설' ? 'text-orange-400' : 'text-zinc-200'}`}>
                                                {item.Name.replace(" 각인서", "").replace("유물 ", "").replace(" 융화 재료", "")}
                                            </p>
                                            <p className="text-[10px] text-zinc-500 font-bold uppercase">{item.Grade}</p>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-base lg:text-lg font-black text-amber-400">{item.CurrentMinPrice.toLocaleString()}G</p>
                                    </div>
                                </div>
                            ))
                        ) : (
                ...
                {/* ➡️ 우측: 시세 비교 막대 그래프 섹션 */}
                <div ref={scrollRef} className="bg-zinc-900/50 border border-white/5 rounded-[2rem] lg:rounded-[2.5rem] flex flex-col p-6 lg:p-10 shadow-2xl backdrop-blur-sm min-h-[400px] lg:h-full">
                    {selectedItem ? (
                        <div className="flex flex-col h-full">
                            ...
                            {/* 막대 그래프 영역 */}
                            <div className="flex-1 min-h-0 bg-black/20 rounded-2xl lg:rounded-[2rem] p-4 lg:p-6 border border-white/5 relative">
