        ...
        <aside className="w-full lg:w-72 lg:sticky lg:top-24 h-fit z-40 shrink-0 lg:mr-8 transition-all duration-500">
            ...
            <div
                className={`
                    ...
                    lg:rounded-[1.5rem] lg:p-6 p-4 rounded-[1.5rem]
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 gap-4">
                            ...
                            <div className="grid grid-cols-2 gap-2 p-1 bg-black/50 rounded-xl border border-white/5 shadow-inner">
                                {(['combat-power', 'item-level'] as const).map((type) => (
                                    <button
                                        ...
                                        className={`relative py-2.5 lg:py-3 text-[12px] lg:text-[13px] font-black rounded-lg transition-all duration-300 ${
                                            rankingType === type
                        ...
                        <div className="space-y-2" ref={dropdownRef}>
                            ...
                            <button
                                ...
                                className={`relative w-full bg-black/40 border transition-all rounded-xl py-3.5 lg:py-4 pl-10 lg:pl-12 pr-10 text-[12px] lg:text-[13px] font-bold text-left outline-none ${
                                    isDropdownOpen ? "border-indigo-500/50 ring-4 ring-indigo-500/5 text-white" : "border-white/5 text-zinc-200"
                                } shadow-inner`}
                            >
                            ...
                            <AnimatePresence>
                                {isDropdownOpen && (
                                    <motion.div
                                        ...
                                        className="overflow-hidden bg-black/40 border border-white/5 rounded-xl mt-1"
                                    >
                                        <div className="overflow-y-auto custom-scrollbar-thin py-2 max-h-[300px] lg:max-h-[400px]">
                                            {classList.map((cls) => (
                                                <div key={cls} className="px-2">
                                                    <button
                                                        onClick={() => handleClassClick(cls)}
                                                        className={`w-full flex items-center justify-between px-4 py-3 text-[12px] font-bold rounded-lg transition-all mb-0.5 ${
                                                    ...
                                                    <AnimatePresence>
                                                        {expandedClass === cls && (
                                                            <motion.div
                                                                ...
                                                                animate={{ height: "auto", opacity: 1 }}
                                                                exit={{ height: 0, opacity: 0 }}
                                                                className="overflow-hidden bg-white/5 rounded-lg mb-1"
                                                            >
                                                                <div className="py-1 px-2 space-y-0.5">
                                                                    {arkPassiveList.map((passive) => (
                                                                        <button
                                                                            ...
                                                                            className={`w-full flex items-center gap-3 px-6 py-2.5 rounded-md text-[11px] font-bold transition-all ${
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative group">
                                ...
                                <input
                                    ...
                                    className="w-full bg-black/40 border border-white/5 rounded-xl py-3.5 lg:py-4 pl-10 lg:pl-12 pr-4 text-[12px] lg:text-[13px] font-bold text-zinc-200 placeholder:text-zinc-700 outline-none focus:border-indigo-500/50 focus:ring-4 focus:ring-indigo-500/5 transition-all shadow-inner"
                                />
                            </div>
                        </div>
                    </div>
